...
function CompanyPage() {
  ...
  const calculateSDGBudget = (sdgNumber) => {
    ...
    return company.project_initiatives
      ...
      .reduce((total, project) => total + (Number.parseFloat(project.budget) || 0), 0)
  }
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-primary-foreground/80 text-lg">{company.industry || "Industry not specified"}</p>
                </div>
              </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <Button variant="ghost" size="icon" onClick={() => setEditAboutOpen(true)}>
                  <Edit className="h-4 w-4" />
                </Button>
              </div>
              <p className="text-muted-foreground mb-6">{company.description || "No description available"}</p>
                ...
                <ul className="space-y-4">
                  {company.sdg_initiatives.map((sdg) => (
                    <li key={sdg.id} className="flex items-start gap-4">
                      <div
                        className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${getSDGColor(sdg.number)}`}
                      >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-2">
                          <p className="text-sm font-medium text-green-600">
                            <span className="text-muted-foreground">Project Spend:</span> ₹
            ...
            <Card className="p-6">
              ...
              {company.project_initiatives && company.project_initiatives.length > 0 ? (
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <div
                                className={`px-3 py-1 rounded-full text-sm font-medium ${getSDGColor(project.sdg_number)}`}
                              >
                                SDG {project.sdg_number}
                              </div>
                              <div
                                className={`px-3 py-1 rounded-full text-sm font-medium ${getProjectStatusColor(project.project_status)}`}
                              >
                                {project.project_status}
                              </div>
                            </div>
                            <h3 className="text-lg font-semibold text-gray-900 mb-2">{project.program_name}</h3>
                            <p className="text-gray-600 text-sm leading-relaxed">{project.modalities}</p>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="flex items-center gap-3 text-gray-600">
                            ...
                            <div>
                              <p className="text-xs text-gray-500">Implementing Organisation</p>
                              <p className="text-sm font-medium">{project.implementing_organisation}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-3 text-gray-600">
                            ...
                            <div>
                              ...
                              <p className="text-sm font-medium">₹{project.budget} Cr</p>
                            </div>
                          </div>
                        ...
                        {project.details && project.details !== "Not applicable" && (
                          <div className="mt-4 pt-4 border-t border-gray-100">
                            <p className="text-sm text-gray-600">
                              <span className="font-medium text-gray-700">Additional Details:</span> {project.details}
                            </p>
                          </div>
                        )}
                      </div>
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <Card className="p-6">
              <div className="flex justify-between items-start mb-4">
                ...
                <Button variant="ghost" size="icon" onClick={() => setEditCSROpen(true)}>
                  <Edit className="h-4 w-4" />
                </Button>
              </div>
              {company.csr_policy ? (
                <div className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">CSR Policy Document</p>
                    <p className="text-xs text-muted-foreground truncate">{company.csr_policy.split("/").pop()}</p>
                  </div>
                  <a
                ...
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  ...
                  <p className="text-muted-foreground">No CSR policy available</p>
                  <Button variant="outline" size="sm" className="mt-3" onClick={() => setEditCSROpen(true)}>
                    Upload Policy
                  </Button>
                </div>
              )}
      ...
      />
      <EditCSRDialog company={company} onSave={handleCompanyUpdate} open={editCSROpen} onOpenChange={setEditCSROpen} />
    </div>
  )
}
